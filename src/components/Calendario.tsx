...
export const Calendario: React.FC = () => {
    ...
    // ▼▼▼ MODIFICA: Rimossa la logica "initialScrollDone" per rendere lo scroll sempre attivo al cambio di vista ▼▼▼
    useEffect(() => {
        // Esegui solo se la vista è 'calendar' e le altre condizioni sono soddisfatte
        if (posts.length > 0 && viewMode === 'calendar' && mainHeaderHeight > 0) {
            // Un piccolo timeout per dare al DOM il tempo di aggiornarsi dopo il cambio di vista
            setTimeout(() => {
    ...
    useEffect(() => { if (viewMode !== 'calendar' || !PERPETUAL_SCROLL_ENABLED || visibleWeeksCount >= allWeeks.length) return; const observer = new IntersectionObserver( (entries) => { if (entries[0].isIntersecting) { setVisibleWeeksCount(prevCount => Math.min(prevCount + 4, allWeeks.length)); } }, { rootMargin: "500px", threshold: 0.1 } ); const currentRef = loadMoreRef.current; if (currentRef) { observer.observe(currentRef); } return () => { if (currentRef) { observer.unobserve(currentRef); } }; }, [viewMode, allWeeks.length, visibleWeeksCount, PERPETUAL_SCROLL_ENABLED]);
    ...
    const handleFilterClick = (categoria: Categoria) => { setViewMode('list'); setFilterCategory(categoria); };
    ...
    if (userLoading || !user) { return <div className="text-center p-10">Caricamento calendario...</div>; }
    ...
    return (
        <div>
            <div id="stats-header" className="sticky z-20 bg-gray-100 dark:bg-gray-900 pb-4" style={{ top: `${mainHeaderHeight}px` }}><div className="mt-6"> <Stats posts={posts} progetti={progetti} onFilterClick={handleFilterClick} onNewPostClick={() => setIsAddModalOpen(true)} onImportClick={() => setIsImportModalOpen(true)} onExportClick={handleExport} onProjectsClick={() => setIsProjectModalOpen(true)} /> </div></div>
            <div className="mt-6"> <DndContext onDragEnd={handleDragEnd} > {viewMode === 'calendar' ? ( isDesktop ? <DesktopView /> : <MobileView />) : ( <FilteredListView posts={postsDaCreareFiltrati} progetti={progetti} filterCategory={filterCategory as Categoria} onBack={handleShowCalendar} onPostClick={handleCardClick} onStatusChange={handleStatusChange}/> )} </DndContext> {viewMode === 'calendar' && (PERPETUAL_SCROLL_ENABLED ? visibleWeeksCount < allWeeks.length : false) && (<div ref={loadMoreRef} className="h-20 flex items-center justify-center text-gray-400"><p>Caricamento...</p></div>)} </div>
