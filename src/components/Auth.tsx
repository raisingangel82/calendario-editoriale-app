...
import { auth, db } from '../firebase'; // Importiamo anche 'db'
import { 
...
// Importiamo le funzioni di Firestore per scrivere il profilo utente
...
export const Auth: React.FC = () => {
  ...
  // Funzione per creare il documento utente nel database
    ...
    try {
      if (isLogin) {
        ...
        if (currentUser && currentUser.isAnonymous) {
          // Caso 1: L'utente era anonimo, colleghiamo l'account
          const credential = EmailAuthProvider.credential(email, password);
          const userCredential = await linkWithCredential(currentUser, credential);
          console.log("Account anonimo collegato con successo!");
          // Creiamo il suo profilo nel database
          await createUserProfileDocument(userCredential);
        } else {
          // Caso 2: Un nuovo utente si registra da zero
          const userCredential = await createUserWithEmailAndPassword(auth, email, password);
          console.log("Nuovo account creato con successo!");
          // Creiamo il suo profilo nel database
          await createUserProfileDocument(userCredential);
        }
      }
    } catch (err: any) {
      // Gestione degli errori più leggibile
