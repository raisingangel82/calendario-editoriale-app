...
export const ProjectManagerModal: React.FC<ProjectManagerModalProps> = ({ progetti, onClose, onAddProject, onUpdateProject, onDeleteProject }) => {
  ...
  const handleSave = () => { if (nome.trim() === '') { alert('Il nome del progetto non può essere vuoto.'); return; } const projectData = { nome: nome.trim(), color: color }; if (editingId) { onUpdateProject(editingId, projectData); } else { onAddProject(projectData); } resetForm(); };
  const handleStartEdit = (proj: Progetto) => setEditingId(proj.id);
  const handleDelete = (projectId: string, projectName: string) => { if (window.confirm(`Sei sicuro di voler eliminare il progetto "${projectName}"? Questa azione è irreversibile.`)) onDeleteProject(projectId); };

  const inputStyle = "w-full p-3 bg-gray-100 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-base text-gray-900 dark:text-gray-100";
  ...
  const footerContent = (
    <div className="flex w-full justify-end gap-3">
        ...
        <button onClick={handleSave} className={`px-6 py-3 text-base font-medium text-white rounded-lg transition-colors flex items-center gap-2 ${getActiveColor('bg')} hover:${getActiveColor('bg', '600')}`}>
    ...
    <BaseModal isOpen={true} onClose={onClose} title="Gestisci Progetti" footer={footerContent}>
      <div className="space-y-8">
          ...
          <div>
              ...
              {progetti.length > 0 ? (<ul className="space-y-2">{progetti.map(proj => {
                ...
                return (
                  <li key={proj.id} className="flex items-center justify-between p-3 bg-gray-100 dark:bg-slate-700/50 rounded-lg">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => handleDelete(proj.id, proj.nome)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-full" title="Elimina"><Trash2 size={16}/></button>
                    </div>
                  </li>
                );
              })}</ul>) : (<p className="text-sm text-center text-gray-500 dark:text-gray-400 py-4">Nessun progetto creato.</p>)}
          </div>
      </div>
    </BaseModal>
  );
