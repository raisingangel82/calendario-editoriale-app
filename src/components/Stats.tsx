...
// Funzione di categorizzazione riscritta per essere più precisa e robusta
const getCategoriaGenerica = (tipoContenuto: string): Categoria => {
    ...
    switch (tipo) {
        ...
        case 'Vlog':
            return 'Video';
        // Tutti gli altri tipi, incluso "Testo breve con Immagine", ricadono qui
        default:
            return 'Testo';
    }
};
...
export const Stats: React.FC<StatsProps> = ({ posts, progetti, onFilterClick }) => {
    const stats = useMemo(() => {
        ...
        posts.forEach(post => {
            ...
            if (post.libro && totaliPerProgetto.hasOwnProperty(post.libro)) {
                    ...
                    <div className="space-y-2">
                        {progetti.map(proj => (
                            <div key={proj.id} className="text-xs">
                                ...
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5"><div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${((stats.creatiProgetto[proj.nome] || 0) / (stats.totaliProgetto[proj.nome] || 1)) * 100}%` }}></div></div>
                            </div>
                        ))}
                    </div>
                </div>
