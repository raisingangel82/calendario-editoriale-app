    ...
    return (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-lg h-full">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-6 gap-y-4">
                ...
                {/* Colonna 1: Produzione per Progetto */}
                ...
                {/* Colonna 2: Produzione per Categoria (con layout modificato) */}
                <div className="lg:col-span-1 lg:border-l lg:pl-6 border-gray-200 dark:border-gray-700">
                    <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
                        <BarChart2 size={16} /> Produzione per Categoria
                    </h3>
                    {/* ▼▼▼ MODIFICA: 'grid-cols-3' diventa 'space-y-3' per impilare verticalmente ▼▼▼ */}
                    <div className="space-y-3">
                        {/* ▼▼▼ MODIFICA: Cambiato l'ordine in Testo, Immagine, Video ▼▼▼ */}
                        {(['Testo', 'Immagine', 'Video'] as const).map(cat => (
                            <div key={cat} className="text-xs">
                                <button onClick={() => onFilterClick(cat)} className="w-full text-left group" disabled={stats.totaliPerCategoria[cat] === 0}>
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-bold text-gray-600 dark:text-gray-400 group-hover:text-red-600 dark:group-hover:text-red-500 transition-colors">{cat}</span>
                                        <span className="text-gray-500 dark:text-gray-500">{stats.creatiPerCategoria[cat]}/{stats.totaliPerCategoria[cat]}</span>
                                    </div>
                                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                                        <div className="bg-red-500 h-1.5 rounded-full" style={{ width: `${(stats.creatiPerCategoria[cat] / (stats.totaliPerCategoria[cat] || 1)) * 100}%` }}></div>
                                    </div>
                                </button>
                            </div>
                        ))}
