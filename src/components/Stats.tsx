...
export const Stats: React.FC<StatsProps> = ({ posts, progetti, onFilterClick, onNewPostClick, onImportClick, onExportClick, onProjectsClick }) => {
    ...
    const ProduzioneProgetto = ({ isScrollable = false }) => ( 
        <div className="relative h-full flex flex-col">
            ...
            <div className={`space-y-2 pr-2 ${isScrollable ? 'max-h-[150px] overflow-y-auto custom-scrollbar' : 'flex-grow'}`}>
                {progetti.map(proj => { 
            ...
            {isScrollable && <div className={`absolute bottom-0 left-0 right-0 h-8 pointer-events-none ${theme === 'light' ? 'bg-gradient-to-t from-white' : 'bg-gradient-to-t from-gray-800'}`}></div>}
        </div> 
    );
    ...
    return (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-lg h-full flex flex-col">
            ...
            {isDesktop ? (
                // LAYOUT DESKTOP
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-6 gap-y-4 h-full">
                    <div className="lg:col-span-1"><ProduzioneProgetto isScrollable={false} /></div>
                    <div className="lg:col-span-2 lg:border-l lg:pl-6 border-gray-200 dark:border-gray-700 flex flex-col">
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 flex-grow">
                            <div className="space-y-4"><ProjectFilter /><ProduzioneCategoria /></div>
                            <div><Riepilogo /><ActionButtons isDesktop={true} /></div>
                        </div>
                    </div>
                </div>
            ) : (
