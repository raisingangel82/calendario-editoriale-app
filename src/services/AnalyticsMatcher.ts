...
    },
    'tiktok': {
        date: 'Post time', // Aggiornato
        views: 'Total views', // Aggiornato
        likes: 'Total likes', // Aggiornato
        comments: 'Total comments', // Aggiornato
        shares: 'Total shares', // Aggiunto
        title: 'Video title', // Aggiunto
        link: 'Video link' // Aggiunto per il link del video
    }
};
...
    const platformName = platform.toLowerCase();
    ...
    // Leggi solo per la piattaforma TikTok (come richiesto nella conversazione precedente)
    if (platformName !== 'tiktok') {
        console.log(`Processing skipped for platform '${platformName}'. Only 'tiktok' is allowed.`);
...
        const commentsValue = getValueFromRecord(record, mapper.comments);
        const sharesValue = getValueFromRecord(record, mapper.shares); // Aggiunto per TikTok
        if (!viewsValue && !likesValue && !commentsValue && !sharesValue) continue; // Salta se non ci sono dati di performance.
        
...
        if (sharesValue) performanceData.shares = parseInt(String(sharesValue).replace(/,/g, ''), 10) || 0; // Inclusione shares

        const postTitle = getValueFromRecord(record, mapper.title);
        const postDescription = getValueFromRecord(record, mapper.description); // Questo è 'Didascalia' per IG, 'Descrizione' per FB, nulla per TikTok
        const postType = getValueFromRecord(record, mapper.postType);
        const postLink = getValueFromRecord(record, mapper.link); // Aggiunto per il link del video
...
            
            // Logica di aggiornamento titolo/descrizione/testo
            if (platformName === 'tiktok' && postTitle != null) { // Specifico per TikTok: usa Video title come titolo e testo
                postUpdateData.titolo = postTitle;
                postUpdateData.descrizione = postTitle; // Usa il titolo come descrizione/testo per TikTok
                postUpdateData.testo = postTitle;
            } else if (platformName === 'facebook' && postDescription != null) {
                postUpdateData.titolo = postDescription;
            } else if (postTitle != null) {
                postUpdateData.titolo = postTitle;
            }

            // Aggiorna descrizione e testo per piattaforme diverse da TikTok
            if (platformName !== 'tiktok' && postDescription != null) {
...
            }
            ...
            // Aggiungi il link se disponibile
            if (postLink != null) {
                postUpdateData.link = postLink;
            }
            
...
            const newPostData = {
                userId,
                // Logica di assegnazione del titolo
...
                descrizione: (platformName === 'tiktok' && postTitle) || postDescription || "Testo non disponibile nel file.", // Per TikTok, usa il titolo come descrizione
                testo: (platformName === 'tiktok' && postTitle) || postDescription || "Testo non disponibile nel file.", // Per TikTok, usa il titolo come testo
...
            }
            ...
            // Aggiungi il link al nuovo post se disponibile
            if (postLink != null) {
                (newPostData as any).link = postLink;
            }
